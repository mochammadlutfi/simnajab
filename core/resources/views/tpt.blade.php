...
<div class="content">
    <nav class="breadcrumb bg-white push">
        ...
        <span class="breadcrumb-item active">TPT</span>
    </nav>
    <div class="block">
        <div class="block-header block-header-default">
            <h3 class="block-title">Kelola <small>Data TPT</small></h3>
            @role('admin')
        ...
        <div class="modal-content">
            <div class="block block-rounded mb-0">
                ...
                <div class="block-content pt-0">
                    <form id="form-tpt" class="form-horizontal">
                        @csrf
                        <input type="hidden" name="tpt_id" value="">
                        <div class="row">
                            <div class="col-lg-12">
...
jQuery(document).ready(function () {
    ...
    $(document).on('click', '#btn_tambah', function () {
        save_method = 'tambah';
        $('#form-tpt')[0].reset();
        $('#modal_title').text('Tambah TPT Baru');
        $('#modal_form').modal({
            backdrop: 'static',
            keyboard: false
        });
    ...
    $("#form-tpt").submit(function (e) {
        ...
        if (save_method == 'tambah') {
            link = "<?= route('tpt.simpan'); ?>";
            pesan = "Data Jalan Baru Berhasil Ditambahkan";
        } else {
            link = laroute.route('tpt.update');
...
function edit(id){
    ...
    $.ajax({
        url : laroute.route('tpt.edit', {id : id}),
        type: "GET",
        dataType: "JSON",
        success: function(response)
        {
            $('[name="tpt_id"]').val(response.tpt_id);
            ...
            $('#modal_title').text('Perbaharui Data TPT');
            $('#modal_form').modal({
                backdrop: 'static',
                keyboard: false
            });
...
function hapus(id) {
    ...
    .then((willDelete) => {
        if (willDelete) {
        $.ajax({
            url: laroute.route('tpt.hapus', { id: id }),
            type: "get",
            dataType: "JSON",
            success: function(data) {
                swal({
                    title: "Berhasil",
                    text: "Data TPT Berhasil Dihapus",
                    timer: 3000,
                    buttons: false,
                    icon: 'success',
