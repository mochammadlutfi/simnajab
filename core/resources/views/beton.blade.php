...
<div class="content">
    <nav class="breadcrumb bg-white push">
        ...
        <span class="breadcrumb-item active">Flat Beton</span>
    </nav>
    <div class="block">
        <div class="block-header block-header-default">
            <h3 class="block-title">Kelola <small>Data Flat Beton</small></h3>
            @role('admin')
                <button id="btn_tambah" type="button" class="btn btn-secondary mr-5 mb-5 float-right btn-rounded">
                    <i class="si si-plus mr-5"></i>
                    Tambah Flat Beton Baru
                </button>
            @endrole
        </div>
        <div class="block-content">
            <table class="table table-hover table-striped" id="list-beton">
                <thead>
                    <tr>
                        <th>No</th>
        ...
        <div class="modal-content">
            <div class="block block-rounded mb-0">
                ...
                <div class="block-content pt-0">
                    <form id="form-beton" class="form-horizontal">
                        @csrf
                        <input type="hidden" name="beton_id" value="">
                        <div class="row">
                            <div class="col-lg-12">
...
jQuery(document).ready(function () {
    ...
    $(document).on('click', '#btn_tambah', function () {
        save_method = 'tambah';
        $('#form-beton')[0].reset();
        $('#modal_title').text('Tambah Flat Beton Baru');
        $('#modal_form').modal({
            backdrop: 'static',
            keyboard: false
        })
    ...
    $("#form-beton").submit(function (e) {
        ...
        if (save_method == 'tambah') {
            link = "<?= route('beton.simpan'); ?>";
            pesan = "Data Flat Beton Baru Berhasil Ditambahkan";
        } else {
            link = laroute.route('beton.update');
            pesan = "Data Flat Beton Berhasil Diperbaharui";
        }
...
function edit(id){
    ...
    $.ajax({
        url : laroute.route('beton.edit', {id : id}),
        type: "GET",
        dataType: "JSON",
        success: function(response)
        {
            $('[name="beton_id"]').val(response.beton_id);
            $('#field-jalan_id').val(response.jalan_id).trigger('change');
            $('[name="kondisi"]').val(response.kondisi);
            $('#modal_title').text('Perbaharui Data Flat Beton Baru');
            $('#modal_form').modal({
                backdrop: 'static',
                keyboard: false
            });
...
function hapus(id) {
    ...
    .then((willDelete) => {
        if (willDelete) {
        $.ajax({
            url: laroute.route('beton.hapus', { id: id }),
            type: "get",
            dataType: "JSON",
            success: function(data) {
                swal({
                    title: "Berhasil",
                    text: "Data Flat Beton Berhasil Dihapus",
                    timer: 3000,
                    buttons: false,
                    icon: 'success',
