...
<div class="content">
    <nav class="breadcrumb bg-white push">
        ...
        <span class="breadcrumb-item active">{{ $step2['tujuan'] }} Jembatan</span>
    </nav>
    <div class="block">
        <div class="block-header block-header-default">
            <h3 class="block-title">{{ $step2['tujuan'] }} Jembatan <small>{{ $jalan->nama }}</small></h3>
        </div>
...
jQuery(document).ready(function () {
    ...
    $("#form-jembatan").submit(function (e) {
        ...
        $.ajax({
            url: "{{ route('jembatan.penganggaran') }}",
            type: 'post',
            data: formData,
            cache: false,
            contentType: false,
